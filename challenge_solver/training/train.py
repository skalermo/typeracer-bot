...
from src.utils import encode_text_batch, decode_predictions, correct_prediction, find_last_exp_dir, setup_exp_dirs, \
    char2idx
...
if __name__ == '__main__':
    ...
    train_loader, valid_loader, test_loader, vocabulary = get_dataloaders(data_path, test_data_path, batch_size=batch_size)
    num_chars = len(char2idx)
    print(num_chars)

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(device)
    model = CRNN(num_chars, rnn_hidden_size=args.rnn_hidden_size)
    ...
    for epoch in range(start_epoch, args.num_epochs + 1):
        ...
        logging.info(f'Validation accuracy: {valid_acc} %')
